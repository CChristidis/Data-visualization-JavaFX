...
public class ValueFromCountryAndIndicatorDAOMySQLImpl implements ValueFromCountryAndIndicatorDAO {
	...
	private Map<List<Integer>, Long> indicatorYearlyValueForCountry;
	
	public ValueFromCountryAndIndicatorDAOMySQLImpl() {
		connection = new Connector();
		indicatorYearlyValueForCountry = new HashMap<List<Integer>, Long>();
	}

	
	@Override
	public Map<List<Integer>, Long> readValueFromCountryAndIndicator(List<Integer> id_country, List<Integer> id_ind, List<Integer> years) {
 		...
 		for (Integer country: id_country) {
			...
			for (Integer indicator: id_ind) {
				...
				for (Integer year: years) {
					...
					country_indicator_year = new ArrayList<Integer>(Arrays.asList(country, indicator, 0));
				}
			}
		}
 		// System.out.println(Arrays.toString(indicatorYearlyValueForCountry.entrySet().toArray()));
